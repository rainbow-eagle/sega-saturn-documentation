...
> **_Remarque:_** nous avons réalisé une comparatif de la taille des binaires produits avec GCC-14.2 et le GCC-8.2 utilisé par Jo Engine. Quand les 48 démos de SGL sont testées, en moyenne, les deux compilateurs produisent des binaires de tailles similaires (0,02% plus petits avec GCC14.2 - démo de taille moyenne de 110ko). Quand les 42 démos de Jo Engine sont testées, en moyenne, GCC-14.2 va produire un binaire 8,71% plus lourd (avec une taille moyenne de 153ko). C'est une grosse différence, mais il convient de noter que les démos sont de tout petits jeux qui ne permettent pas d'étudier le comportement du compilateur sur un code plus volumineux et représentatif d'un jeu réel. Il n'est pas exclu que l'_overhead_ produit par GCC-14.2 soit relativement constant et donc représente quelque chose de négligeable à l'échelle d'un jeu plus lourd. À l'inverse, il se peut que tout le code produit s'alourdisse de 9%, indépendamment de la taille du jeu. Par ailleurs, sur les démo SGL, on peut même remarquer que les plus grandes d'entre elles produisent un exécutable plus petit avec GCC-142. Finalement, les optimisations utilisées par Jo Engine n'étaient pas utilisées sur les démos SGL. Une étude plus approfondie portée sur des projets plus volumineux est nécessaire pour tirer des conclusions exploitables.
...
> **_Remarque:_** : utiliser la librarie SGL de Sega demande un peu plus de travail que seulement correctement placer son code de démarrage. Il convient également de demander au linker de placer une autre section de SGLAREA.O et d'autres sources de SGL entre des symboles précis pour que le code de démarrage les retrouve facilement. C'est le sens de l'extrait du _linker script_ de Jo Engine illustré ci-dessous : rassembler toutes les sections "SLPROG" dans une unique section "SLPROG" au sein du binaire produit, nommer le début de la section "__slprog_start" et nommer la fin de la section "slprog_end". Cela va permettre au code de démarrage de retrouver un autre morceau de code de SGL (depuis SGLAREA.O et LIBSGL.A) en mémoire pour le copier ailleurs.
